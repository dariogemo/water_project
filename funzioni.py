...
def na_to_median_target(df : pd.DataFrame, 
                        column : str):
    '''
...
def na_to_median(df : pd.DataFrame, 
                 column : str):
    '''
...
def na_to_norm_distr(df: pd.DataFrame, 
                     column : str):
    '''
...
def k_fold_accuracy(
    ...
    for x in lista:
        ...
        results = cross_val_score(clf, X1, y1, cv = kf)
